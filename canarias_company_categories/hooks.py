...
# Real Canarias Conectada business taxonomy, seeded into the OCA company
# categories (res.company.category) WITHOUT ir.model.data entries: the user
# fully owns the records afterwards (rename/delete them freely) and module
# updates will never recreate or overwrite them. Roots become "view"
# categories (grouping only); leaves are "normal" (assignable to companies,
# one per company). Names are business data of the Canarias marketplace,
# hence kept in Spanish on purpose.
...
def post_init_hook(env):
    """Seed the Canarias taxonomy as company categories, skipping existing names."""
    category_model = env["res.company.category"].with_context(active_test=False)
    roots = {}
    ...
    for name, parent_name in DEFAULT_CATEGORIES:
        ...
        if not existing:
            child_vals.append({"name": name, "parent_id": parent.id, "type": "normal"})
    category_model.create(child_vals)
    _logger.info(
        "Seeded company categories: %s roots, %s children created",
        len(root_vals),
        len(child_vals),
    )
