...
Se identificaron y resolvieron tres problemas críticos en el módulo Memoria Viva:
...
### Síntoma
Al acceder a `https://canariasconectada.es/memoria-viva/mirador-las-coloradas` se obtenía un error 404.
...
```
https://guanarteme.canariasconectada.es/memoria-viva/mirador-las-coloradas
```

### Verificación
```sql
SELECT id, name, slug, state, website_primario_id 
FROM memoria_viva_historia 
...
### Causa Raíz (Fase 1)
El campo `comentarios_publicos` en `memoria_viva_settings` estaba en `false` (valor por defecto).

### Solución (Fase 1)
Activar la configuración en la base de datos:
```sql
UPDATE memoria_viva_settings 
SET comentarios_publicos = true;
```

### Causa Raíz (Fase 2)
Después de activar la configuración, los comentarios seguían sin verse. El problema estaba en el JavaScript (`memoria_viva_comments.js`):
...
**Archivo:** `views/memoria_viva_templates.xml`
...
**Archivo:** `static/src/js/memoria_viva_comments.js`
...
### Causa Raíz
Existían **vistas duplicadas** de "Memoria Viva - Detalle" en la base de datos:

```sql
SELECT id, name, arch_db IS NULL as arch_is_null, arch_fs
FROM ir_ui_view 
WHERE name = 'Memoria Viva - Detalle';
```
...
-------+------------------------+--------------+-----------------------------------------------
 11526 | Memoria Viva - Detalle | t            |                                               
 11548 | Memoria Viva - Detalle | f            | memoria_viva/views/memoria_viva_templates.xml 
```
...
FROM memoria_viva_settings;
```

### API de Comentarios (Prueba)
```bash
curl "https://guanarteme.canariasconectada.es/memoria-viva/comentario/listar?lugar_id=3"
```
...
1. **`views/memoria_viva_templates.xml`**
   - Agregado input `#lugar_id` fuera del bloque condicional de usuarios logueados

2. **`static/src/js/memoria_viva_comments.js`**
...
2. Acceder a: https://guanarteme.canariasconectada.es/memoria-viva/mirador-las-coloradas
...
git checkout -- views/memoria_viva_templates.xml
git checkout -- static/src/js/memoria_viva_comments.js
```
