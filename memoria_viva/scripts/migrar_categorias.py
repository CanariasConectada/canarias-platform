...
def main():
    ...
    categorias = models.execute_kw(DB, uid, PASS, 'memoria.viva.categoria', 'search_read', [
        [], ['id', 'name', 'active']
    ])
    
    # Obtener las nuevas categorías creadas
    cat_nuevas = models.execute_kw(DB, uid, PASS, 'memoria.viva.categoria', 'search_read', [
    ...
    for cat_vieja in categorias:
        ...
        if nombre_nuevo and nombre_nuevo in cat_nuevas_dict:
            ...
            historias = models.execute_kw(DB, uid, PASS, 'memoria.viva.historia', 'search', [
            ...
            if historias:
                ...
                models.execute_kw(DB, uid, PASS, 'memoria.viva.historia', 'write', [
                    historias, {'categoria_id': cat_nueva_id}
                ])
                historias_actualizadas += len(historias)
            
            # Desactivar categoría vieja
            models.execute_kw(DB, uid, PASS, 'memoria.viva.categoria', 'write', [
