...
with open("/home/odoo/addons/memoria_viva/views/memoria_viva_templates.xml", "r") as f:
    content = f.read()
    
# Verificar que el template existe en el archivo
if "memoria_viva_detail" in content:
...
if view:
    ...
    try:
        ...
        tree = etree.parse("/home/odoo/addons/memoria_viva/views/memoria_viva_templates.xml")
        root = tree.getroot()
        for template in root.findall(".//template"):
            template_id = template.get("id")
            if template_id == "memoria_viva_detail":
                print(f"Found template in XML: {template_id}")
                # Crear el view manualmente
                name = template.get("name", "Memoria Viva - Detalle")
                arch = etree.tostring(template, encoding='unicode')
                view_vals = {
                    'name': name,
                    'key': f'memoria_viva.{template_id}',
                    'type': 'qweb',
                    'arch_db': arch,
                }
