...
def test_backend():
    ...
    try:
        result = subprocess.run([
            ...
            "SELECT 1 FROM ir_model_fields WHERE model = 'memoria.viva.historia' AND name = 'anio_foto';"
...
def test_frontend():
    ...
    try:
        resp = session.get(f"{BASE_URL}/memoria-viva", headers=headers, timeout=10)
        html = resp.text
...
def test_api_ordenacion():
    ...
    for orden in ordenes:
        try:
            resp = session.get(f"{BASE_URL}/memoria-viva?ordenar={orden}", 
    ...
    # Test filtro por año
    try:
        resp = session.get(f"{BASE_URL}/memoria-viva?anio=1980", 
                         headers=headers, timeout=10)
        if resp.status_code == 200:
