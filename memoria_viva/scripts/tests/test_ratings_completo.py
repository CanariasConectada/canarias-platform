...
def test_backend_modelo():
    ...
    try:
        ...
        model_ids = models.execute_kw(ODOO_DB, uid, ADMIN_PASS,
            'ir.model', 'search', [[['model', '=', 'memoria.viva.rating']]])
        
        if model_ids:
            log("BACKEND", "Modelo memoria.viva.rating existe", True, f"ID: {model_ids[0]}")
        else:
            log("BACKEND", "Modelo memoria.viva.rating existe", False, "Modelo no encontrado")
        
        # Verificar campos del modelo
        campos = models.execute_kw(ODOO_DB, uid, ADMIN_PASS,
            'memoria.viva.rating', 'fields_get', [], {'attributes': ['string', 'type']})
        ...
        constraints = models.execute_kw(ODOO_DB, uid, ADMIN_PASS,
            'memoria.viva.rating', '_sql_constraints', [])
...
def test_backend_campos_computados():
    ...
    try:
        ...
        lugar = models.execute_kw(ODOO_DB, uid, ADMIN_PASS,
            'memoria.viva.historia', 'read', [[LUGAR_ID]], 
...
def test_api_endpoints():
    ...
    try:
        resp = session.post(
            f"{ODOO_URL}/memoria-viva/rating/enviar",
    ...
    try:
        resp = session.get(
            f"{ODOO_URL}/memoria-viva/rating/mi-valoracion?lugar_id={LUGAR_ID}",
            timeout=10
        )
    ...
    try:
        resp = session.get(
            f"{ODOO_URL}/memoria-viva/{LUGAR_SLUG}",
            timeout=10
        )
...
def test_frontend_render():
    ...
    try:
        ...
        checks = [
            ...
            ('memoria_viva_rating.js', 'Script JS incluido'),
...
def test_seguridad():
    ...
    try:
        ...
        access_rules = models.execute_kw(ODOO_DB, uid, ADMIN_PASS,
            ...
            [[['model_id.model', '=', 'memoria.viva.rating']]],
