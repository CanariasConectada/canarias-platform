...
"""Jubila la copia del cuestionario Silver Economy que traía este módulo.

El cuestionario pasa a tener un solo dueño, ``company_certification``, y la
marca ``is_silver_economy`` se pone ahora sobre el suyo. La copia que
publicaba este módulo queda sin función.
...
respuestas, pero el sistema antiguo llega a este punto con 3 en la suya, y esta
...
OLD_SURVEY = "silver_economy.silver_economy_master_survey"
NEW_SURVEY = "company_certification.survey_silver_economy"
...
def migrate(cr, version):
    ...
    if not new_survey:
        ...
        _logger.error(
            "%s no existe: se deja la copia de silver_economy tal cual", NEW_SURVEY
        )
        return
    new_survey.is_silver_economy = True

    old_survey = env.ref(OLD_SURVEY, raise_if_not_found=False)
    if not old_survey:
        return

    old_survey.is_silver_economy = False

    answers = env["survey.user_input"].search_count([("survey_id", "=", old_survey.id)])
    if answers:
        old_survey.active = False
        _logger.warning(
            "El cuestionario Silver duplicado (id=%s) conserva %s respuestas: "
            "se desmarca y se archiva, pero NO se borra. Revisar a mano si esas "
            "evaluaciones deben pasarse al cuestionario %s.",
            old_survey.id,
            answers,
            new_survey.id,
        )
    ...
    _logger.info(
        "Borrando el cuestionario Silver duplicado (id=%s, sin respuestas); "
        "el dueño pasa a ser %s",
        old_survey.id,
        new_survey.id,
    )
    old_survey.unlink()
