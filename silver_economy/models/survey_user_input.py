...
class SurveyUserInput(models.Model):
    ...
    def action_start_new_silver_evaluation(self):
        """Start a new Silver Economy evaluation from the backend."""
    ...
    def action_start_silver_from_form(self):
        ...
        self.ensure_one()
        return self.action_start_new_silver_evaluation()
    ...
    def _silver_evaluation_domain(self):
        return [
    ...
    def _cron_silver_retry_reminder(self):
        ...
        evaluations = self.search(
            self._silver_evaluation_domain()
            + [
        ...
        template = self.env.ref(
            "silver_economy.mail_template_silver_retry_available",
    ...
    def _cron_silver_renewal_reminder(self):
        ...
        template = self.env.ref(
            "silver_economy.mail_template_silver_renewal_due",
            raise_if_not_found=False,
        )
        if not template:
            return
        ...
        for survey in surveys:
            reminder_days = survey.silver_renewal_reminder_days or 30
            evaluations = self.search(
                self._silver_evaluation_domain()
                + [
                    ("survey_id", "=", survey.id),
    ...
    def _cron_silver_expiry_alert(self):
        ...
        template = self.env.ref(
            "silver_economy.mail_template_silver_admin_expiry_alert",
            raise_if_not_found=False,
        )
        if not template:
            return
    ...
    def _send_silver_badge_notification(self):
        """Notify the user that a new badge was awarded."""
        self.ensure_one()
        if self.certification_level == "none":
            return
        template = self.env.ref(
            "silver_economy.mail_template_silver_new_badge",
            raise_if_not_found=False,
        )
