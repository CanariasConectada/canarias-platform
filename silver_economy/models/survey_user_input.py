...
class SurveyUserInput(models.Model):
    ...
    # inheriting survey.user_input (e.g. company_certification). When those
    # modules are co-installed, this class shadows their compute methods
    # for EVERY record, so records that do not belong to a Silver Economy
    # survey are handed back to the shadowed implementation with
    # ``super(SurveyUserInput, other_inputs)``. The super() call is inlined
    # (not factored into a shared helper) on purpose: a helper with the
    # same name in several verticals would always dispatch from the top of
    # the MRO again and recurse forever.
    @api.depends('scoring_total', 'survey_id', 'is_manually_overridden')
    def _compute_certification_level(self):
        silver_inputs = self.filtered(lambda ui: ui.survey_id.is_silver_economy)
        other_inputs = self - silver_inputs
        if other_inputs:
        ...
        for user_input in silver_inputs:
            ...
            max_score = survey.silver_max_score or 80.0

            if max_score <= 0 or score < (survey.silver_bronze_min or 40):
                user_input.certification_level = 'none'
            elif score <= (survey.silver_silver_min or 55):
                user_input.certification_level = 'bronze'
            elif score <= (survey.silver_gold_min or 70):
    ...
    def _compute_next_attempt_date(self):
        ...
        for user_input in silver_inputs:
            if user_input.state != 'done':
                ...
                continue
            survey = user_input.survey_id
            cooldown = survey.silver_cooldown_months or 3
            validity = survey.silver_validity_years or 1
    ...
    def _compute_expiry_date(self):
        ...
        for user_input in silver_inputs:
            if user_input.state != 'done' or user_input.certification_level == 'none':
                ...
                validity = user_input.survey_id.silver_validity_years or 1
    ...
    @api.model
    def action_start_new_silver_evaluation(self):
        """Acción para iniciar una nueva evaluación Silver Economy desde el backend"""
        user = self.env.user
        ...
        if not survey:
            raise UserError(_('No hay un cuestionario Silver Economy configurado. Contacte al administrador.'))
        
        return survey.action_start_silver_evaluation()
    ...
    def _mark_done(self):
        ...
        for user_input in self:
            if user_input.survey_id.is_silver_economy and user_input.company_id:
                # Invalidar y recomputar certificación de la empresa
                user_input.company_id.invalidate_recordset(fnames=['silver_certification_level', 'silver_certification_date', 'silver_expiry_date', 'silver_cert_score'])
                user_input.company_id._compute_silver_certification()
                user_input.company_id._compute_silver_cert_details()
    ...
    def write(self, vals):
        ...
        if any(f in vals for f in ['certification_level', 'state', 'expiry_date', 'survey_id', 'company_id', 'test_entry']):
            for record in self.filtered(lambda r: r.survey_id.is_silver_economy and r.company_id):
                record.company_id._compute_silver_certification()
                record.company_id._compute_silver_cert_details()
                record.company_id.flush_recordset()
        return res

    def unlink(self):
        """Al eliminar una evaluación, recalcular certificación de la empresa."""
        companies = self.filtered(lambda r: r.survey_id.is_silver_economy).mapped('company_id')
        res = super().unlink()
        for company in companies:
            company._compute_silver_certification()
            company._compute_silver_cert_details()
            company.flush_recordset()
        return res

    @api.model
    def _cron_silver_retry_reminder(self):
        ...
        template = self.env.ref('silver_economy.mail_template_silver_retry_available', raise_if_not_found=False)
        if template:
    ...
    def _cron_silver_renewal_reminder(self):
        ...
        surveys = self.env['survey.survey'].search([('is_silver_economy', '=', True)])
        max_days = max(surveys.mapped('silver_renewal_reminder_days') or [30])
        ...
        template = self.env.ref('silver_economy.mail_template_silver_renewal_due', raise_if_not_found=False)
        if template:
            for evaluation in evaluations:
                if evaluation.partner_id.email:
                    try:
    ...
    def _cron_silver_expiry_alert(self):
        ...
        template = self.env.ref('silver_economy.mail_template_silver_admin_expiry_alert', raise_if_not_found=False)
        if template:
            for evaluation in evaluations:
    ...
    def _send_new_badge_notification(self):
        ...
        self.ensure_one()
        if not self.survey_id.is_silver_economy:
            # Not a Silver Economy evaluation: let the shadowed engine
        ...
        if self.certification_level == 'none':
            return
        template = self.env.ref('silver_economy.mail_template_silver_new_badge', raise_if_not_found=False)
        if template and self.partner_id.email:
            template.send_mail(self.id, force_send=True)

    def action_continue_silver_evaluation(self):
    ...
    def action_start_silver_from_form(self):
        """Inicia una evaluación Silver Economy desde el formulario de creación.
