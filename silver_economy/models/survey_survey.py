...
OWNER_SURVEY_XMLID = "company_certification.survey_silver_economy"
OWNER_TYPE_XMLID = "company_certification.certification_type_silver"
OWNER_TYPE_CODE = "silver"
...
class SurveySurvey(models.Model):
    ...
    silver_positive_item_ids = fields.One2many(
        ...
        help="Items que aparecerán en el microsite cuando el usuario responda "
        "correctamente la pregunta asociada",
    )

    # ------------------------------------------------------------------
    # La marca is_silver_economy se cura sola
    # ------------------------------------------------------------------
    # `is_silver_economy` no es un dato del negocio: es un índice
    ...
    # security/silver_economy_security.xml hay tres reglas que lo consultan,
    # más los controladores públicos y una quincena de búsquedas en
    ...
    @api.model
    def _silver_economy_owner_survey(self):
        """Devuelve el cuestionario que ES la evaluación Silver Economy.
    ...
    @api.model
    def _ensure_silver_economy_flag(self):
        ...
        try:
            ...
            with self.env.cr.savepoint():
                owner = surveys._silver_economy_owner_survey()
                flagged = surveys.search([("is_silver_economy", "=", True)])
                if not owner:
                    _logger.error(
                        "Silver Economy: no se puede resolver el cuestionario "
                        "dueño (ni %s ni %s ni un certification.type con "
                        "code=%s). La marca is_silver_economy se deja como "
                ...
                if intruders:
                    _logger.warning(
                        "Silver Economy: %s cuestionario(s) llevan la marca "
                        "is_silver_economy sin ser el dueño (%s); se les "
                    ...
                    intruders.write({"is_silver_economy": False})
                if owner not in flagged:
                    _logger.warning(
                        "Silver Economy: el cuestionario %r (id=%s) había "
                        "perdido la marca is_silver_economy; se restaura. "
                        "Mientras faltaba, la vertical estaba muda: sin "
                        "evaluaciones visibles y sin sellos.",
                        owner.title,
                        owner.id,
                    )
                    owner.write({"is_silver_economy": True})
                return owner
        except Exception:
            _logger.exception(
                "Silver Economy: fallo al asegurar la marca "
                "is_silver_economy. La vertical puede estar degradada en "
                "silencio; revisar qué cuestionario la lleva."
            )
            return self.browse()
    ...
    def action_start_silver_evaluation(self):
        ...
        if not self.is_silver_economy:
            raise UserError(
                _("Esta encuesta no está configurada como evaluación Silver Economy.")
            )
