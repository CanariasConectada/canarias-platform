# Limpieza de Menús Duplicados - Lugares de Interés

## 📋 Problema Detectado

**Fecha:** 7 de Abril de 2026  
**Síntoma:** Menús de Lugares de Interés aparecían duplicados en el menú principal de Odoo
...
│
└── Lugares de Interés (único menú raíz)
    ├── 📄 Contenido
    │   ├── Lugares
    │   ├── Eventos
    │   └── Anuncios
...
```sql
-- Verificar menús huérfanos de Lugares de Interés
SELECT 
    id,
    name->>'en_US' as name,
    parent_id,
...
Todos los menús deben tener `parent_id` asignado (no NULL), excepto "Lugares de Interés" que es el menú raíz.

---

## 📝 Notas

1. **Los menús nuevos** (IDs 573-590) están correctamente anidados bajo "Lugares de Interés"
