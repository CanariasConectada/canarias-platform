...
```
lugares.interes.historia
    │
    ├── has many ──► lugares.interes.comentario
    │
    └── has many ──► lugares.interes.like

lugares.interes.comentario
    │
    ├── belongs to ──► lugares.interes.historia
    ├── belongs to ──► res.users (autor)
    ├── belongs to ──► lugares.interes.comentario (parent_id)
    └── has many ──► lugares.interes.comentario (respuesta_ids)

lugares.interes.palabra.prohibida
    └── belongs to ──► lugares.interes.settings
```
...
```python
comentario = env['lugares.interes.comentario'].create({
    'lugar_id': 1,
    'autor_id': user.id,
...
El modelo `lugares.interes.comentario` hereda de `mail.thread`:
...
CREATE TABLE lugares_interes_comentario (
    id SERIAL PRIMARY KEY,
    lugar_id INTEGER REFERENCES lugares_interes_historia(id),
    parent_id INTEGER REFERENCES lugares_interes_comentario(id),
...
CREATE TABLE lugares_interes_palabra_prohibida (
    id SERIAL PRIMARY KEY,
    name VARCHAR NOT NULL,
    active BOOLEAN DEFAULT TRUE,
    settings_id INTEGER REFERENCES lugares_interes_settings(id)
);
```
