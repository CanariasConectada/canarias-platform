...
def test_anonimo():
    ...
    try:
        resp = session.get(f"{BASE_URL}/lugares-de-interes/{LUGAR_SLUG}", 
    ...
    try:
        resp = session.post(f"{BASE_URL}/lugares-de-interes/rating/enviar",
...
def test_backend():
    ...
    try:
        result = subprocess.run([
            ...
            '-c', "SELECT model FROM ir_model WHERE model = 'lugares.interes.rating';"
        ], capture_output=True, text=True, timeout=10)
        
        if 'lugares.interes.rating' in result.stdout:
    ...
    try:
        result = subprocess.run([
            'sudo', '-u', 'postgres', 'psql', '-d', 'canarias_conectada',
            '-c', "SELECT name FROM ir_model_fields WHERE model = 'lugares.interes.rating';"
        ], capture_output=True, text=True, timeout=10)
    ...
    try:
        result = subprocess.run([
            'sudo', '-u', 'postgres', 'psql', '-d', 'canarias_conectada',
            '-c', f"SELECT rating_avg, rating_count FROM lugares_interes_historia WHERE id = {LUGAR_ID};"
        ], capture_output=True, text=True, timeout=10)
    ...
    try:
        result = subprocess.run([
            'sudo', '-u', 'postgres', 'psql', '-d', 'canarias_conectada',
            '-c', "SELECT perm_read, perm_create FROM ir_model_access WHERE model_id = (SELECT id FROM ir_model WHERE model = 'lugares.interes.rating');"
        ], capture_output=True, text=True, timeout=10)
...
def test_template():
    ...
    elementos = [
        ...
        ('lugares_interes_rating.js', 'Script JS'),
    ...
    try:
        for elemento, descripcion in elementos:
            result = subprocess.run([
                'sudo', '-u', 'postgres', 'psql', '-d', 'canarias_conectada',
                '-c', f"SELECT 1 FROM ir_ui_view WHERE name = 'Lugares de Interés - Detalle' AND arch_db::text LIKE '%{elemento}%';"
            ], capture_output=True, text=True, timeout=10)
...
def test_api_funcional():
    ...
    try:
        resp = session.get(f"{BASE_URL}/lugares-de-interes/comentario/listar?lugar_id={LUGAR_ID}",
                          headers=headers, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
    ...
    try:
        resp = session.get(f"{BASE_URL}/lugares-de-interes/rating/mi-valoracion?lugar_id={LUGAR_ID}",
                          headers=headers, timeout=10)
