# Guía de Pruebas - Lugares de Interés
...
2. `tests/test_lugares_interes_models.py` - Tests para modelos (lugares, likes, configuración)
3. `tests/test_lugares_interes_comentarios.py` - Tests para sistema de comentarios
4. `tests/test_lugares_interes_website.py` - Tests para controladores web
5. `scripts/test_xmlrpc.py` - Script de prueba XML-RPC
6. `scripts/test_orm.py` - Script de prueba ORM directo

#### JavaScript Frontend
- `static/src/js/lugares_interes_comments.js` - Sistema de comentarios completo

#### Correcciones Realizadas
- `__manifest__.py` - Corregida referencia al archivo JS
- `views/lugares_interes_templates.xml` - Template limpio sin código residual
...
**URL:** `https://guanarteme.canariasconectada.es/lugares-de-interes/<slug>`
...
**Ruta:** `Aplicaciones > Lugares de Interés > Comentarios`
...
**Ruta:** `Aplicaciones > Lugares de Interés > Palabras Prohibidas`
...
python3 addons/lugares_interes/scripts/test_xmlrpc.py

# Con credenciales personalizadas
python3 addons/lugares_interes/scripts/test_xmlrpc.py http://localhost:8069 canarias_conectada admin admin
```
...
python3 addons/lugares_interes/scripts/test_orm.py
```
...
Si los modelos `lugares.interes.comentario` y `lugares.interes.palabra.prohibida` no aparecen:

```sql
-- Verificar en base de datos
SELECT model FROM ir_model WHERE model LIKE 'lugares.interes%';

-- Si faltan, forzar actualización del módulo
UPDATE ir_module_module SET state='to upgrade' WHERE name='lugares_interes';
```

Luego reiniciar Odoo con `-u lugares_interes`
...
```bash
./odoo-bin -u lugares_interes -d canarias_conectada --stop-after-init
```
...
./odoo-bin -i lugares_interes -d canarias_conectada --test-enable \
    --stop-after-init --test-tags=lugares_interes
```
...
1. **Frontend JavaScript** - `lugares_interes_comments.js` completo
2. **Templates** - Limpios y funcionales
3. **Tests** - Suite completa de pruebas
4. **Scripts** - XML-RPC y ORM para verificación

### ⚠️ Requiere Atención
Los modelos `lugares.interes.comentario` y `lugares.interes.palabra.prohibida` pueden requerir:
- Reinicio de Odoo con `-u lugares_interes`
