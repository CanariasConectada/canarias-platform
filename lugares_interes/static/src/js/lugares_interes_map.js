...
odoo.define('lugares_interes.map', function (require) {
    'use strict';

    try {
        var publicWidget = require('web.public.widget');
    } catch (e) {
        console.warn('[LugaresInteres] web.public.widget no disponible, mapa no cargado');
        return;
    }
    ...
    // MAPA PRINCIPAL
    // ========================================
    publicWidget.registry.LugaresInteresMap = publicWidget.Widget.extend({
        selector: '#lugares-de-interes-map',
        ...
        _loadMap: function () {
            ...
            // Leer datos
            var dataElement = document.getElementById('lugares-de-interes-lugares-data');
            this.lugaresData = [];
            ...
            if (dataElement) {
                try {
                    ...
                    console.error('[Lugares de Interés] Error parsing lugares data:', e);
                }
            }
        ...
        _initGoogleMap: function () {
            ...
            this.lugaresData.forEach(function (lugar) {
                ...
                var infowindow = new google.maps.InfoWindow({
                    content: '<div style="max-width:200px;"><h6>' + lugar.name + '</h6><a href="/lugares-de-interes/' + lugar.slug + '">Ver →</a></div>',
                });
            ...
            console.log('[Lugares de Interés] Google Maps inicializado');
        },
        ...
        _initLeafletMap: function () {
            ...
            this.lugaresData.forEach(function (lugar) {
                var marker = L.marker([lugar.lat, lugar.lng])
                    .addTo(map)
                    .bindPopup('<strong>' + lugar.name + '</strong><br><a href="/lugares-de-interes/' + lugar.slug + '">Ver historia →</a>');
                markers.push(marker);
            });
            ...
            console.log('[Lugares de Interés] OpenStreetMap inicializado con', this.lugaresData.length, 'marcadores');
        },
    });
    ...
    // FORMULARIO
    // ========================================
    publicWidget.registry.LugaresInteresForm = publicWidget.Widget.extend({
        selector: '#form-lugares-de-interes',
        ...
        _onSubmit: function (ev) {
            ...
            reader.onload = function (e) {
                ...
                // Enviar
                $.ajax({
                    url: '/lugares_interes/api/submit',
                    type: 'POST',
    ...
    return {
        LugaresInteresMap: publicWidget.registry.LugaresInteresMap,
        LugaresInteresForm: publicWidget.registry.LugaresInteresForm,
    };
});
