...
class LugaresInteresComentario(models.Model):
    _name = 'lugares.interes.comentario'
    _description = 'Comentario - Lugares de Interés'
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            palabras_prohibidas = self.env['lugares.interes.palabra.prohibida'].search([
                ('active', '=', True)
            ])
        ...
        comentarios = super(LugaresInteresComentario, self).create(vals_list)
    ...
    def _notificar_moderacion(self):
        ...
        try:
            grupo_aprobadores = self.env.ref('lugares_interes.group_lugares_interes_approver', raise_if_not_found=False)
        ...
        for usuario in usuarios_aprobadores:
            self.env['mail.activity'].create({
                ...
                'user_id': usuario.id,
                'res_id': self.id,
                'res_model_id': self.env['ir.model']._get_id('lugares.interes.comentario'),
            })
        
        # Enviar email
        template = self.env.ref('lugares_interes.email_template_comentario_moderacion', raise_if_not_found=False)
        if template:
            for usuario in usuarios_aprobadores:
                if usuario.email:
