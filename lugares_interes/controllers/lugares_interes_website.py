...
class LugaresInteresWebsite(http.Controller):
    """Controlador Website para Lugares de Interés - Exclusivo Guanarteme"""

    def _get_allowed_websites(self):
        """Busca y retorna los websites permitidos para Lugares de Interés"""
        domains = [
            'guanarteme.canariasconectada.es',
            'tamaraceite.canariasconectada.es',
            'lomolosfrailes.canariasconectada.es',
            'canariasconectada.es',
        ]
        websites = request.env['website'].sudo().search([
            ('domain', 'ilike', 'guanarteme.canariasconectada.es'),
        ]) | request.env['website'].sudo().search([
            ('domain', 'ilike', 'tamaraceite.canariasconectada.es'),
        ]) | request.env['website'].sudo().search([
            ('domain', 'ilike', 'lomolosfrailes.canariasconectada.es'),
        ]) | request.env['website'].sudo().search([
            ('domain', 'ilike', 'canariasconectada.es'),
        ])
        return websites

    def _check_allowed_website(self):
        """Verifica que estemos en un website permitido"""
        allowed = self._get_allowed_websites()
        if not allowed:
    ...
    # ========================================
    @http.route('/lugares-de-interes', auth='public', website=True)
    def lugares_interes_list(self, **kw):
        """Página principal: Hero + Filtros + Mapa + Galería + Formulario"""
        self._check_allowed_website()
        
        # Obtener parámetros de filtro
        search = kw.get('search', '')
        tipo_filter = kw.get('tipo')
        categoria_filter = kw.get('categoria')
        barrio_filter = kw.get('barrio')
        ordenar = kw.get('ordenar', 'default')
        
        # Construir dominio base (solo lugares aprobados con al menos una foto)
        Historia = request.env['lugares.interes.historia'].sudo()
        image_domain = ['|', ('image_main', '!=', False), ('image_ids', '!=', False)]
        domain = ['&', ('state', '=', 'aprobado')] + image_domain
        
        # Filtro automático por microsite según dominio de acceso
        current_website = request.website
        if current_website.id == 228:
            # Canarias Conectada (principal): solo lugares SIN microsite asignado
            domain = ['&'] + domain + ['&', ('website_primario_id', '=', False), ('website_ids', '=', False)]
        else:
            # Microsite específico: lugares asignados a este microsite
            domain = ['&'] + domain + ['|', ('website_primario_id', '=', current_website.id), ('website_ids', 'in', [current_website.id])]
        
        # Dominio base para conteos (mismo filtro de microsite)
        base_count_domain = list(domain)
        ...
        # Filtro por barrio
        if barrio_filter:
            domain = ['&'] + domain + [('barrio', 'ilike', barrio_filter)]
        ...
        for lugar in lugares:
            if lugar.latitude and lugar.longitude:
                lugares_data.append({
                    ...
                    'slug': lugar.slug,
                    'image': f'/web/image/lugares.interes.historia/{lugar.id}/image_main'
                })
        lugares_json = json.dumps(lugares_data)
        
        # Obtener tipos y categorías para filtros (con conteos)
        Tipo = request.env['lugares.interes.tipo'].sudo()
        Categoria = request.env['lugares.interes.categoria'].sudo()
        
        tipos = Tipo.search([('active', '=', True)])
        categorias = Categoria.search([('active', '=', True), ('tipo_id', '=', 11)])  # Solo tipo General
        
        # Preparar tipos y categorías con conteos (mismo dominio base de microsite + imágenes)
        tipos_data = []
        for tipo in tipos:
            count = Historia.search_count(['&'] + base_count_domain + [('tipo_id', '=', tipo.id)])
            tipos_data.append({'id': tipo.id, 'name': tipo.name, 'count': count})
        
        categorias_data = []
        for cat in categorias:
            count = Historia.search_count(['&'] + base_count_domain + [('categoria_id', '=', cat.id)])
        ...
        Settings = request.env['lugares.interes.settings'].sudo()
        config = Settings.get_settings()
        
        # Barrios disponibles para filtro
        barrios = ['Guanarteme', 'Tamaraceite', 'Lomo Los Frailes', 'La Isleta', 'Vegueta']
        
        # Calcular conteo de filtros activos
        filtros_activos = 0
        if tipo_filter: filtros_activos += 1
        if categoria_filter: filtros_activos += 1
        if barrio_filter: filtros_activos += 1
        ...
        likes_usuario = []
        if session_id:
            Like = request.env['lugares.interes.like'].sudo()
        ...
        def make_url(exclude=None):
            ...
            if barrio_filter and exclude != 'barrio':
                parts.append(f'barrio={barrio_filter}')
        ...
        url_quitar_barrio = make_url('barrio')
        
        return request.render('lugares_interes.lugares_interes_list', {
            'lugares': lugares,
            'search': search or '',
            'tipos': tipos_data,
            ...
            'url_quitar_barrio': url_quitar_barrio,
            'page': page,
            'limit': limit,
            'total': total,
    ...
    # PÁGINA DE DETALLE
    # ========================================
    @http.route('/lugares-de-interes/<string:slug>', auth='public', website=True)
    def lugares_interes_detail(self, slug, **kw):
        ...
        Anuncio = request.env['lugares.interes.anuncio'].sudo()
        ...
        return request.render('lugares_interes.lugares_interes_detail', {
            'lugar': lugar,
    ...
    # ========================================
    @http.route('/lugares_interes/api/historias', auth='public', methods=['GET', 'POST'], type='json', csrf=False)
    def api_list(self, **kw):
        ...
        for lugar in lugares:
            items.append({
                ...
                'slug': lugar.slug,
                'image_main': f'/web/image/lugares.interes.historia/{lugar.id}/image_main',
    ...
    # ========================================
    @http.route('/lugares_interes/api/submit', auth='public', methods=['POST'], type='json', csrf=False)
    ...
    # ========================================
    @http.route('/lugares-de-interes/anuncio/click/<int:anuncio_id>', auth='public', type='http')
    def anuncio_click(self, anuncio_id, **kw):
        ...
        return werkzeug.utils.redirect('/lugares-de-interes')

    # ========================================
    # LIKE A LUGAR
    # ========================================
    @http.route('/lugares-de-interes/like/<int:lugar_id>', auth='public', type='http', csrf=False)
    def like_lugar(self, lugar_id, **kw):
        ...
        try:
            Lugar = request.env['lugares.interes.historia'].sudo()
            Like = request.env['lugares.interes.like'].sudo()
            Settings = request.env['lugares.interes.settings'].sudo().get_settings()
    ...
    # COMENTARIOS
    # ========================================
    @http.route('/lugares-de-interes/comentario/enviar', auth='user', type='http', csrf=False, methods=['POST'])
    def enviar_comentario(self, **kw):
        ...
        try:
            ...
            # Crear comentario
            Comentario = request.env['lugares.interes.comentario'].sudo()
            vals = {
    ...
    @http.route('/lugares-de-interes/comentario/listar', auth='public', type='http', methods=['GET'])
    def listar_comentarios(self, lugar_id, offset=0, limit=10, **kw):
        ...
        try:
            ...
            Config = request.env['lugares.interes.settings'].sudo().get_settings()
    ...
    # ========================================
    @http.route('/lugares-de-interes/rating/enviar', auth='user', type='http', csrf=False, methods=['POST'])
    def enviar_rating(self, **kw):
        ...
        try:
            ...
            Rating = request.env['lugares.interes.rating'].sudo()
    ...
    @http.route('/lugares-de-interes/rating/eliminar', auth='user', type='http', csrf=False, methods=['POST'])
    ...
    @http.route('/lugares-de-interes/rating/mi-valoracion', auth='user', type='http', methods=['GET'])
