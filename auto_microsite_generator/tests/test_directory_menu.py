...
class TestMicrositeDirectoryMenu(TransactionCase):
    ...
    def _migration_script(self):
        spec = importlib.util.spec_from_file_location("post_migration", MIGRATION_2_3_0)
    ...
    def test_the_sweep_clears_the_merchant_and_spares_the_platform_sites(self):
        ...
        counts = self.Company._remove_microsite_directory_menus(
            zone | named | portal | merchant, zone_companies=zone.company_id
        )

        self.assertEqual(counts["deleted"], 1)
        self.assertEqual(counts["spared_platform_site"], 3)
    ...
    def test_the_sweep_spares_an_aggregated_shop(self):
        ...
        # Straight into the column, which is what the sweep reads: through
        # the ORM the flag backfills a whole marketplace (carrier, products).
        self.env.cr.execute(
            "UPDATE website SET is_marketplace = TRUE WHERE id = %s", (aggregated.id,)
        )
        ...
        self.assertEqual(counts["spared_platform_site"], 1)
    ...
    def test_the_sweep_is_idempotent(self):
        ...
        self.assertEqual(counts["deleted"], 0, "a second run finds nothing left")
    ...
    def test_an_entry_with_children_is_somebody_s_navigation(self):
        ...
        with self.assertLogs(MODEL_LOGGER, level="INFO") as capture:
            counts = self.Company._remove_microsite_directory_menus(
        ...
        self.assertEqual(counts["deleted"], 0)
        self.assertEqual(counts["skipped_has_children"], 1)
        self.assertTrue(
            any("skipped_has_children" in message for message in capture.output),
            "what is kept must be logged with its reason",
        )
    ...
    def test_an_entry_moved_under_a_dropdown_is_kept(self):
        ...
        self.assertEqual(counts["skipped_not_top_level"], 1)
    ...
    def test_nothing_goes_when_no_zone_can_be_told_apart(self):
        ...
        with patch.object(
            ...
            counts = self.Company._remove_microsite_directory_menus(merchant)
        self.assertTrue(entry.exists())
        self.assertEqual(sum(counts.values()), 0)
    ...
    def test_the_migration_finds_the_zone_sites_without_the_orm_field(self):
        """The registry is incomplete while a post-migration runs: the zone
        companies must come from SQL, so a blind ORM probe changes nothing.
        """
